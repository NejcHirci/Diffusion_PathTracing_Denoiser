...
class DiffusionModelForDenoising(torch.nn.Module):
    ...
    def forward(self, x_real, x_gt):
        ...
        #predicted_gt = self.denoiser(noisy_x, t, return_dict=False)[0]
        predicted_gt = self.denoiser(noisy_x, t)

        return predicted_gt, x_gt

    def sample(self, x_real):
        ...
        for t in range(self.num_timesteps-1, -1, -1):
            ...
            factor_t0 = factor_t0.to(device)
            factor_t1 = factor_t1.to(device)
            pred_x = self.denoiser(x_t, time_tensor)
            #pred_x = self.denoiser(x_t, time_tensor, return_dict=False)[0]
